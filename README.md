...
> **Status: Phase 11 — Power BI Executive Dashboard (complete, as a real `.pbip` project)**
...
**Phase 11 result:** a real **Power BI Project** (`.pbip`) —
[`powerbi/EcommerceExecutiveDashboard.pbip`](powerbi/EcommerceExecutiveDashboard.pbip)
— not a fabricated `.pbix` (which cannot be authored outside Power BI
Desktop; see [`docs/powerbi_design_spec.md`](docs/powerbi_design_spec.md)
for why). Opening it in Power BI Desktop loads a real semantic model: 8
tables, all relationships, and all 12 DAX measures matching the KPI
framework exactly. 4 report pages are wired to the model and ready for
visuals, which the design spec documents page-by-page. **File → Save As →
.pbix** produces a genuine Power BI file. See
[`powerbi/README.md`](powerbi/README.md) for setup steps.
